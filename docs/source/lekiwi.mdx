...
The current LeKiwi example records locally by default. In this repository, `examples/lekiwi/record.py` stores the dataset under:

```text
/home/puffy/lerobot/lerobot/datasets/lekiwi-trash
```

Make sure to first adapt the `remote_ip`, `HF_REPO_ID`, and `TASK_DESCRIPTION` values in `examples/lekiwi/record.py`.
...
The LeKiwi example scripts in this repo are configured to use the local `datasets/` folder rather than the default Hugging Face cache. The main paths are:

- Recording target: `/home/puffy/lerobot/lerobot/datasets/lekiwi-trash`
- Converted third-party dataset example: `/home/puffy/lerobot/lerobot/datasets/lekiwi_pen`

If you want to convert an older `v2.1` LeRobot dataset before training, use:
...
To replay an episode run the API example below, make sure to change `remote_ip`, dataset id and episode index as needed.
...
Before training, it is a good idea to quality-check the recorded episodes:

```bash
python examples/lekiwi/qc_dataset_episodes.py
```

If the report flags bad episodes and you want to apply the cleanup:
...
This is a small training run to confirm the dataset, normalization and action ranges all look healthy:
...
lerobot-train \
  ...
  --policy.type=smolvla \
  --output_dir=outputs/train/smolvla_lekiwi_trash_smoke \
  --job_name=smolvla_lekiwi_trash_smoke \
  --policy.device=cuda \
  --wandb.enable=false \
...
If you do not have an NVIDIA GPU, replace `--policy.device=cuda` with `--policy.device=cpu`.
...
lerobot-train \
  ...
  --dataset.root=/home/puffy/lerobot/lerobot/datasets/lekiwi-trash \
  --policy.type=smolvla \
  --output_dir=outputs/train/smolvla_lekiwi_trash \
  --job_name=smolvla_lekiwi_trash \
  --policy.device=cuda \
  --wandb.enable=false \
  --policy.push_to_hub=false \
  --steps=10000 \
...
```bash
lerobot-train \
  --config_path=outputs/train/smolvla_lekiwi_trash/checkpoints/last/pretrained_model/train_config.json \
  --resume=true
```

## Evaluate your policy

To evaluate your policy run the `evaluate.py` API example, make sure to change `remote_ip` and the checkpoint path:

```bash
python examples/lekiwi/evaluate.py \
  --policy-path outputs/train/smolvla_lekiwi_trash/checkpoints/last/pretrained_model \
  --wait-for-host
```
