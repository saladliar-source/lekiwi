...
def _init_rerun(session_name: str = "lerobot_control_loop") -> None:
    ...
    rerun_mode = os.getenv("LEROBOT_RERUN_MODE", "auto")
    is_wsl = "WSL_DISTRO_NAME" in os.environ

    if rerun_mode == "web" or (rerun_mode == "auto" and is_wsl):
        rr.serve(
            open_browser=True,
...
def _maybe_downsample_for_web(arr: np.ndarray) -> np.ndarray:
    max_dim = int(os.getenv("LEROBOT_RERUN_WEB_MAX_DIM", "480"))
    if max_dim <= 0 or arr.ndim != 3:
        return arr
        ...
        for k, v in observation.items():
            ...
            if _is_scalar(v):
                ...
                if arr.ndim == 1:
                    ...
                    if "WSL_DISTRO_NAME" in os.environ or os.getenv("LEROBOT_RERUN_MODE") == "web":
                        arr = _maybe_downsample_for_web(arr)
