...
class KeyboardTeleop(Teleoperator):
    ...
    def __init__(self, config: KeyboardTeleopConfig):
        ...
        self.use_stdin_backend = False
        self.stdin_last_pressed = {}
        self.logs = {}
    ...
    def connect(self) -> None:
        ...
        if backend != "stdin" and PYNPUT_AVAILABLE:
            ...
            logging.info("Using stdin keyboard backend.")
            self._connect_stdin()
    ...
    def _drain_stdin_keys(self):
        ...
        while ready:
            ...
            if key_char:
                ...
                self.stdin_last_pressed[key_char] = now
            ready, _, _ = select.select([sys.stdin], [], [], 0)

        hold_s = getattr(self.config, "stdin_hold_s", 0.25)
        for key_char, last_pressed_t in list(self.stdin_last_pressed.items()):
            if now - last_pressed_t <= hold_s:
                self.current_pressed[key_char] = True
            else:
                self.stdin_last_pressed.pop(key_char, None)
    ...
    def disconnect(self) -> None:
        ...
        if self.listener is not None:
            self.listener.stop()
        if self.stdin_settings is not None and self.stdin_fd is not None:
            termios.tcsetattr(self.stdin_fd, termios.TCSADRAIN, self.stdin_settings)
        self.stdin_fd = None
        self.stdin_settings = None
        self.use_stdin_backend = False
        self.stdin_last_pressed.clear()
