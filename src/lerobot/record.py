...
def record_loop(
    ...
    while control_time_s is None or timestamp < control_time_s:
        ...
        if dataset is not None:
            ...
            observation_frame = {
                key: obs_processed[key.removeprefix("observation.images.")]
                if key.startswith("observation.images.")
                else obs_processed[key]
        ...
        if policy is not None and preprocessor is not None and postprocessor is not None:
            ...
            if dataset is not None:
                ...
                if len(action_values) < len(action_names):
                    action_names = action_names[: len(action_values)]
            ...
            if "\x1b" in keyboard_action:
                logging.info("Escape pressed. Stopping data recording.")
                teleop_keyboard.current_pressed.clear()
                teleop_keyboard.stdin_last_pressed.clear()
                events["stop_recording"] = True
                events["exit_early"] = True
                break
            if "q" in keyboard_action:
                logging.info("Keyboard 'q' pressed. Ending current episode.")
                teleop_keyboard.current_pressed.clear()
                teleop_keyboard.stdin_last_pressed.clear()
                events["exit_early"] = True
                break
            if "r" in keyboard_action:
                logging.info("Keyboard 'r' pressed. Re-recording current episode.")
                teleop_keyboard.current_pressed.clear()
                teleop_keyboard.stdin_last_pressed.clear()
